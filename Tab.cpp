...
Symbol* Tab::NewSym(int typ, std::wstring name, int line) {
	if (CocoUtil::coco_string_length(name) == 2 && name[0] == '"') {
		parser->SemErr(L"empty token not allowed");
		name = CocoUtil::coco_string_create(L"???");
	}
...
Symbol* Tab::FindSym(const std::wstring name) {
	...
	for (i=0; i<terminals->Count; i++) {
		...
		if (CocoUtil::coco_string_equal(s->name, name)) return s;
	}
...
void Tab::PrintSym(Symbol *sym) {
	...
	fwprintf(trace, L"%3d %14s %ls", sym->n, paddedName.c_str(), nTyp[sym->typ]);
	CocoUtil::coco_string_delete(paddedName);
...
void Tab::PrintSet(BitArray *s, int indent) {
	...
	for (int i=0; i<terminals->Count; i++) {
		sym = (Symbol*)((*terminals)[i]);
		if ((*s)[sym->n]) {
			len = CocoUtil::coco_string_length(sym->name);
			if (col + len >= 80) {
				fwprintf(trace, L"\n");
...
Graph* Tab::StrToGraph(const std::wstring str) {
	std::wstring subStr = CocoUtil::coco_string_create(str, 1, CocoUtil::coco_string_length(str)-2);
	std::wstring s = Unescape(subStr);
	CocoUtil::coco_string_delete(subStr);
	if (CocoUtil::coco_string_length(s) == 0) parser->SemErr(L"empty token not allowed");
	Graph *g = new Graph();
	g->r = dummyNode;
	for (int i = 0; i < CocoUtil::coco_string_length(s); i++) {
...
std::wstring Tab::Name(const std::wstring name) {
	std::wstring name2 = CocoUtil::coco_string_create_append(name, L"           ");
	std::wstring subName2 = CocoUtil::coco_string_create(name2, 0, 12);
	CocoUtil::coco_string_delete(name2);
	return subName2;
...
CharClass* Tab::NewCharClass(const std::wstring name, CharSet *s) {
	CharClass *c;
	if (CocoUtil::coco_string_equal(name, L"#")) {
		std::wstring temp = CocoUtil::coco_string_create_append(name.c_str(), (wchar_t) dummyName++);
		c = new CharClass(temp, s);
		CocoUtil::coco_string_delete(temp);
	} else {
...
CharClass* Tab::FindCharClass(const std::wstring name) {
	...
	for (int i=0; i<classes->Count; i++) {
		...
		if (CocoUtil::coco_string_equal(c->name, name)) return c;
	}
	return NULL;
}
...
void Tab::WriteCharClasses () {
	...
	for (int i=0; i<classes->Count; i++) {
		...
		std::wstring format2 = CocoUtil::coco_string_create_append(c->name, L"            ");
		std::wstring format  = CocoUtil::coco_string_create(format2, 0, 10);
		CocoUtil::coco_string_merge(format, L": ");
		fwprintf(trace, format.c_str());

		WriteCharSet(c->set);
		fwprintf(trace, L"\n");
		CocoUtil::coco_string_delete(format);
		CocoUtil::coco_string_delete(format2);
	}
	fwprintf(trace, L"\n");
}
...
wchar_t Tab::Hex2Char(const std::wstring s) {
	int val = 0;
	int len = CocoUtil::coco_string_length(s);
	for (int i = 0; i < len; i++) {
		wchar_t ch = s[i];
...
std::wstring Tab::Unescape (const std::wstring s) {
	...
	while (i < len) {
		if (s[i] == '\\') {
			switch (s[i+1]) {
				...
				case L'u': case L'x':
					if (i + 6 <= CocoUtil::coco_string_length(s)) {
						std::wstring subS = CocoUtil::coco_string_create(s, i+2, 4);
						buf.Append(Hex2Char(subS)); i += 6; break;
						CocoUtil::coco_string_delete(subS);
					} else {
						parser->SemErr(L"bad escape sequence in string or character");
						i = CocoUtil::coco_string_length(s); break;
					}
				default:
						parser->SemErr(L"bad escape sequence in string or character");
					i += 2; break;
...
std::wstring Tab::Escape (const std::wstring s) {
	StringBuilder buf;
	wchar_t ch;
	int len = CocoUtil::coco_string_length(s);
	for (int i=0; i < len; i++) {
...
void Tab::ResErr(Node *p, const std::wstring msg) {
...
void Tab::SetDDT(const std::wstring s) {
	std::wstring st = CocoUtil::coco_string_create_upper(s);
	wchar_t ch;
	int len = CocoUtil::coco_string_length(st);
	for (int i = 0; i < len; i++) {
		ch = st[i];
	...
	CocoUtil::coco_string_delete(st);
}


void Tab::SetOption(const std::wstring s) {
	// example: $namespace=xxx
	//   index of '=' is 10 => nameLenght = 10
	//   start index of xxx = 11

	int nameLenght = CocoUtil::coco_string_indexof(s, '=');
	int valueIndex = nameLenght + 1;

	std::wstring name = CocoUtil::coco_string_create(s, 0, nameLenght);
	std::wstring value = CocoUtil::coco_string_create(s, valueIndex);

	if (CocoUtil::coco_string_equal(L"$namespace", name)) {
		if (nsName.length() == 0) nsName = CocoUtil::coco_string_create(value);
	} else if (CocoUtil::coco_string_equal(L"$checkEOF", name)) {
		checkEOF = CocoUtil::coco_string_equal(L"true", value);
	}
